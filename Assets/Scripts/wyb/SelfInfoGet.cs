...
public class SelfInfoGet : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //生成货物信息的列表
        GameObject floorInfo = GameObject.Find("UI/ShelfInfoWindow/Shelf_Goods/FloorInfo");
        if (floorInfo.activeSelf == true)
        {
            CreateTable(shelfHub);
        }
    }
    ...
    public void CreateTable(ShelfHub shelfHub)
    {
        ...
        int shelfFloor = shelfHub.shelf.floor.Count;
        //生成列表数据
        for (int i = 0; i < shelfHub.shelf.floor[shelfFloor].material.Count; i++)
        {
            ...
            //设置预设实例中的各个子物体的文本内容
            material material = shelfHub.shelf.floor[nowFloor].material[i];
